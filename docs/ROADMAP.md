...
* [ ] Replace argparse's default `-h`/`--help` output with a custom renderer that prints a
  ...
  confusing.
* [ ] Show both German and English together, but as two separate full-width blocks — the
...
* [ ] Needs one new i18n key per command (e.g. `cli.commands.backup.description`) in both
...
* [ ] Implementation stays in `lbm/cli/` (presentation-only, no business logic): intercept
